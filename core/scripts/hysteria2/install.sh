...
install_hysteria() {
    ...
    echo "Updating hysteria-server.service to use SafeConnect Panel config.json..."
    sed -i 's|(config.yaml)|(SafeConnect Panel)|' /etc/systemd/system/hysteria-server.service
