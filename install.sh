...
clone_repository() {
    log_info "Cloning SafeConnect repository..."
    ...
    if git clone https://github.com/behrouz-atoofi/SafeConnect /etc/hysteria &> /dev/null; then
...
run_menu() {
    ...
    echo -e "\n${BOLD}${GREEN}======== Launching SafeConnect Menu ========${NC}\n"
    ./menu.sh
}

main() {
    echo -e "\n${BOLD}${BLUE}======== SafeConnect Setup Script ========${NC}\n"
    ...
    echo -e "\n${YELLOW}Starting SafeConnect in 3 seconds...${NC}"
    sleep 3
